...
- **Always run the Playwright tests locally before pushing.** Run `npx playwright test --project=chromium` and confirm they pass before pushing.
- **Never push directly to `main`.** All changes must go through a feature branch and a pull request. Branch naming: `type/short-description` (e.g. `fix/footer-selector`, `feat/webp-images`). Open a PR with `gh pr create` and let CI pass before merging. Never use `git push origin main` or `git push` while on `main`.
